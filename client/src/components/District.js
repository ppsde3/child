...
function District() {
    ...
    const fetchState = async () => {
      ...
      try {
        const response = await axios({
          method: 'GET',
          url: 'http://15.206.43.124:5000/api/districts?state_id=1',
        });
        if (response.status === 200) {
          setIsFetching(false);
          setData(response.data.district);
        }
      } catch (error) {
        return setAlert(
          <SweetAlert
          ...
          >
            Problems to retrieve the district. Please, try again later.
          </SweetAlert>
        );
      }
    };
  ...
  let results;
  if (data) {
    results = data.map((el, index) => <DistrictListing key={index} {...el} />);
  } else {
    results = (
